class Volunteer < ApplicationRecord
  ...
  def role_available?(role_name, day)
    role_id = Role.find_by(role_name: role_name)&.id
    return false unless role_id

    time_slot = TimeSlot.find_by(day_of_week: day)
    return false unless time_slot

    RoleAssignment.where(role_id: role_id, time_slot_id: time_slot.id).where('people_count < 3').exists?
  end
